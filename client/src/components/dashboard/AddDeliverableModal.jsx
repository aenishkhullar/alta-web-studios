...
export const AddDeliverableModal = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await deliverableService.createDeliverable(payload);
      ...
      if (result.success) {
        ...
        toast.error(result.message || 'Failed to create deliverable.');
      }
    } catch (error) {
  ...
  return (
    <div
      onClick={(e) => e.target.id === 'add-deliv-backdrop' && onClose()}
      id="add-deliv-backdrop"
      style={{
        position: 'fixed',
        inset: 0,
        ...
        <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3 style={{ fontSize: '20px', fontWeight: '700', fontFamily: 'var(--font-heading)', margin: 0 }}>
              New Deliverable Setup
            </h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: '13px', margin: '4px 0 0 0' }}>
              Create a deliverable tracking card for your projects.
            </p>
          </div>
          <button
            onClick={onClose}

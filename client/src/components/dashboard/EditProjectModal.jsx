...
import { X, Briefcase, User, Calendar, Link, Code, DollarSign, AlertCircle, FileText, Activity } from 'lucide-react';
...
export const EditProjectModal = ({ project, onClose, onUpdated }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    techStackRaw: project.techStack ? project.techStack.join(', ') : '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await projectService.updateProject(project._id, payload);
      ...
      const serverMessage = error.response?.data?.message || 'Failed to update project. Please try again.';
  ...
  return (
    <div
      onClick={(e) => e.target.id === 'edit-proj-backdrop' && onClose()}
      id="edit-proj-backdrop"
      style={{
        position: 'fixed',
        inset: 0,
    ...
    >
      ...
      >
        ...
        <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '13px', margin: '4px 0 0 0' }}>
              Modify information for <strong>{project.title}</strong>.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* Row 4: Progress slider */}
          <div>
            <label style={labelStyle}>Project Completion Progress ({formData.progress}%)</label>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', padding: '12px 16px', borderRadius: 'var(--border-radius-md)' }}>
